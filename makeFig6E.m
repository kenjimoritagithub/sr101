...
lrset{3} = [0.15 0.15 0.15 0.15 0.05];
for k1 = 3
    for k2 = 1:n
        fprintf('sim %d-%d\n',k1,k2);
...
save data23030501_estparas estparas
save data23030501_w_flag1 w_flag1

% Wilcoxon rank-sum test (Mann Whitney U test) in R
load data23030501_w_flag1
for k1 = 3
    csvwrite(['data23030501_w_' num2str(k1) '.csv'],w_flag1{k1});
end
% codes in R
%dat1 <- read.csv("C:/###/data22101401_w_1.csv", header=FALSE, sep=",")
dat2 <- read.csv("C:/###/data22101401_w_2.csv", header=FALSE, sep=",")
dat3 <- read.csv("C:/###/data23030501_w_3.csv", header=FALSE, sep=",")
wilcox.test(dat2$V1, dat3$V1)
% <Results in R>
% Wilcoxon rank sum test with continuity correction
% data:  dat2$V1 and dat3$V1
% W = 3964.5, p-value = 0.05857
% alternative hypothesis: true location shift is not equal to 0

% figure
load data23030501_w_flag1
tmpX = [0.05:0.1:0.95];
H{3} = hist(w_flag1{3},tmpX);
Ymax = 55;
for k1 = 3
    F = figure;
    A = axes;
...
save data23030502_choice_pattern_set choice_pattern_set
for k1 = 3
    F = figure;
    A = axes;
    hold on;
