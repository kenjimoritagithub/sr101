function out = OCD_model_rev_decay_scaling(type_of_learning,SR_ap,SR_an,IR_ap,IR_an,b,n,decay_rate)
...
for t = 1:n
    ...
    if t >= 2
        ...
        if RPE >= 0
            ...
            w = w + SR_ap*SR_M(act_t(t-1),:)*RPE/(SR_M(act_t(t-1),:)*SR_M(act_t(t-1),:)');
        else
            IR_Q(act_t(t-1)) = IR_Q(act_t(t-1)) + IR_an*RPE;
            w = w + SR_an*SR_M(act_t(t-1),:)*RPE/(SR_M(act_t(t-1),:)*SR_M(act_t(t-1),:)');
        end
